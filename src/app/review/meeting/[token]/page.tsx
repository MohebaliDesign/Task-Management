...
import { ReviewExperience, type ReviewParticipant, type ReviewDecision, type OwnReviewState } from "@/features/review/review-experience";
import {
  getMeetingByToken, getProject, getMeetingSpace, getMeetingDecisions, getMeetingActions,
  getDependencyViews, getBlockers, getOwnSignature, getPerson,
} from "@/lib/queries";
import { roleLabels } from "@/lib/labels";
import { faDate, toFa, reviewerCookieName } from "@/lib/utils";
...
export default function ReviewPage({ params }: { params: { token: string } }) {
  ...
  // Privacy boundary: resolve ONLY this browser's own identity/state via the
  // per-meeting cookie set on a previous response — never the full reviewer
  // list. See getOwnSignature (lib/queries.ts) and ReviewExperience's doc
  // comment for the full rule (aggregate/other-reviewer visibility is
  // PM-Meeting-Detail-only).
  const myReviewerId = cookies().get(reviewerCookieName(meeting.id))?.value;
  const mySignature = getOwnSignature(meeting.id, myReviewerId);
  const ownState: OwnReviewState | null =
    mySignature && mySignature.status !== "pending"
      ? { status: mySignature.status, signedAtLabel: mySignature.signedAt ? faDate(mySignature.signedAt) : null }
      : null;
...
/** Minimal shell for edge cases with no review flow at all (e.g. invalid token) — no action bar. */
